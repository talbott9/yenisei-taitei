...
void Projectile::shootEnemy4(double x, double y, Chara* hildegarde) {
	...
	if(initialiseLaser) {
		...
		if(initialiseStimulant) {
			for(int i = 0; i < 3; i++) {
			projectiles2[i].posX = defaultPosX; projectiles2[i].posY = defaultPosY;
			projectiles2[i].defaultPosX = defaultPosX; projectiles2[i].defaultPosY = defaultPosY;
			projectiles2[i].mBox.x = posX; projectiles2[i].mBox.y = posY;
			projectiles2[i].speed = speed;
			projectiles2[i].mBox.w = mBox.w;
			projectiles2[i].mBox.h = mBox.h;
			if(shootAttack4Rand == 0) {
				projectiles2[i].targetX = SCREEN_WIDTH/2 - 100; 
				projectiles2[i].targetY = SCREEN_HEIGHT/6;
			} else {
				projectiles2[i].targetX = SCREEN_WIDTH/2 + 100; 
				projectiles2[i].targetY = SCREEN_HEIGHT/6;
			}
		...
		if(checkCollision(projectiles2[2].mBox, camera)) {
			for(int i = 0; i < 3; i++) {
				if(projectiles2[i].dx == 0.0 && projectiles2[i].dy == 0.0) {
					projectiles2[i].posX = defaultPosX; projectiles2[i].posY = defaultPosY;
				}
			}
			projectiles2[0].moveGravity(projectiles2[0].targetX, projectiles2[0].targetY, 0.1);
			...
			if(stimulantTicks >= 30) {
				projectiles2[1].moveGravity(projectiles2[1].targetX, projectiles2[1].targetY, 0.1);
				projectiles2[1].posX += projectiles2[1].dx;
				projectiles2[1].posY += projectiles2[1].dy;
				projectiles2[1].mBox.x = projectiles2[1].posX;
				projectiles2[1].mBox.y = projectiles2[1].posY;
			}
			if(stimulantTicks >= 60) {
				projectiles2[2].moveGravity(projectiles2[2].targetX, projectiles2[2].targetY, 0.1);
				projectiles2[2].posX += projectiles2[2].dx;
				projectiles2[2].posY += projectiles2[2].dy;
				projectiles2[2].mBox.x = projectiles2[2].posX;
				projectiles2[2].mBox.y = projectiles2[2].posY;
			}
		} else {
			for(int i = 0; i < 3; i++) {
			projectiles2[i].posX = defaultPosX;
	...
	if(checkCollision(projectiles2[0].mBox, camera)) {
		if(shootAttack4Rand == 0)
			projectiles2[0].targetX = shootTicks + 100;
		else
			projectiles2[0].targetX = SCREEN_WIDTH - shootTicks - 100;
	...
	for(int i = 0; i < 3; i++) {
		if((projectiles2[i].dx != 0.0 || projectiles2[i].dy != 0.0) && shootStimulant) {
			gTexture->render(projectiles2[i].mBox.x - projectiles2[i].mBox.w/3, projectiles2[i].mBox.y - projectiles2[i].mBox.h/3, NULL);
		} 
	}
	//printf("%i/%i/%b/%i/%i/%i/%i\n", projectiles2[i].dx, projectiles2[i].dy, checkCollision(projectiles2[i].mBox, camera), hildegarde->getBox().x, hildegarde->getBox().y, projectiles2[i].targetX, projectiles2[i].targetY);
	projectileTicks++;
	//printf("%i/%i/%i\n", shootAttack4Rand, projectiles2[0].targetX, shootTicks);
}
